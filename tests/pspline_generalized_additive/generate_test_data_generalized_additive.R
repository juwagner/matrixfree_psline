# ------------------------------------------------------------------------------
# Generate data for generalized additive P-spline tests
# ------------------------------------------------------------------------------
...
alpha_terms <- lapply(1:n_terms, function(s) 0.3*rnorm(prod(J_vec[[s]])))
...
eta <- sin(2*pi*X_terms[[1]][,1]*X_terms[[1]][,2])*cos(2*pi*X_terms[[1]][,3]) +
  0.3*cos(2*pi*X_terms[[2]][,1]*X_terms[[2]][,2])
y <- as.vector(exp(eta) + rnorm(n, sd=0.1))
...
Phi_alpha <- as.vector(Phi %*% alpha)
W1 <- exp(Phi_alpha)
W2 <- exp(2*Phi_alpha)

PhiT_W_Phi_full <- PhiT_full %*% (W2 * Phi)
